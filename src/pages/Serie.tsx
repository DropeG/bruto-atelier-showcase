...
const Serie = () => {
  ...
  // IDs de las imágenes de serie
  const serieIds = [2, 9];
  const serieItems = galleryItems.filter(item => 
    serieIds.includes(item.id)
  );
  ...
  const allImagesLoaded = serieItems.every(item => imagesLoaded[item.id]);
